// OpenCVenc.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
void testVideoStream()
{
    ...
    cv::VideoCapture* slam_cam = InitCamera(0, w, h, 30);

    cv::Size SizeOfFrame = cv::Size(w, h);
    //cv::VideoWriter video("Result.avi", CV_FOURCC('M', 'J', 'P', 'G'), 30, SizeOfFrame, true);
    //cv::VideoWriter video("Result.avi", CV_FOURCC('H', '2', '6', '4'), 30, SizeOfFrame, true);
    cv::VideoWriter video("Result.avi", CV_FOURCC('X', 'V', 'I', 'D'), 30, SizeOfFrame, true);
    //cv::VideoWriter video("Result.avi", CV_FOURCC('M', 'P', 'G', '4'), 30, SizeOfFrame, true);
    std::cout << "VIDEOWRITER_PROP_QUALITY = " << video.get(VideoWriterProperties::VIDEOWRITER_PROP_QUALITY) <<
        "VIDEOWRITER_PROP_FRAMEBYTES = " << video.get(VideoWriterProperties::VIDEOWRITER_PROP_FRAMEBYTES) <<
        "VIDEOWRITER_PROP_NSTRIPES = " << video.get(VideoWriterProperties::VIDEOWRITER_PROP_NSTRIPES) << std::endl;
...
//        bCanBeRead = (fread(pYUV, 1, kiPicResSize, pFileYUV) == kiPicResSize);
        ...
        //Mat mYUV(720 + 720/2, 1280, CV_8UC1, (void*)pYUV);
        //Mat mRGB(720, 1280, CV_8UC3);
        Mat mYUV_back(iSourceHeight + iSourceHeight / 2, iSourceWidth, CV_8UC1, (void*)pYUV);
        //cvtColor(mYUV, mRGB, cv::COLOR_YUV2RGB_YV12, 3);

        //cv::waitKey(1);

        (*slam_cam) >> frame_color;
        //cvtColor(frame_color, frame, cv::COLOR_BGR2GRAY);
        //cvtColor(mYUV, frame, cv::COLOR_YUV2RGB_YV12);

        //cvtColor(frame_color, frame, cv::COLOR_BGR2RGB);
        //cvtColor(frame, mYUV_back,cv::COLOR_RGB2YUV_I420);

        cvtColor(frame_color, mYUV_back, cv::COLOR_BGR2YUV_I420);

        //        cvtColor(frame_color, frame, cv::COLOR_RGB2YUV_I420);
//        frame = cv::imdecode((InputArray)pYUV, cv::IMREAD_REDUCED_COLOR_2);
//
////        Mat tt = frame.t();
//        for (size_t i = 0; i < kiPicResSize; i++)
//        {
////            pYUV[i] = frame.data[i];
//            frame.data[i] = pYUV[i];
//        }

        cv::imshow("Input Frame", frame_color);
        cv::waitKey(1);
